...
def generate_ellipsoid_clusters(distance, n_samples=100, cluster_std=0.5):
    ...
    shift_vector = np.array([distance, distance])  # Uniform shift in both directions
    X2 += shift_vector
...
def do_experiments(start, end, step_num):
    ...
    for i, distance in enumerate(shift_distances, 1):
        ...
        distances = cdist(Z > 0.7, Z <= 0.7, metric='euclidean')
        min_distance = np.min(distances)
        margin_widths.append(min_distance)

        plt.title(f"Shift Distance = {distance}", fontsize=12)
        plt.xlabel("x1")
        plt.ylabel("x2")
        plt.legend()
